...
function createStackScreen(nome, display, screenClass) {
    return (
        <Stack.Screen name={nome} component={screenClass} options={{title: `Calculadora${display ? `: ${display}` : ""}`}}/>
    )
}
...
function App() {
    return ( 
        <NavigationContainer>
            <Stack.Navigator>
                {createStackScreen("Operations", "Operações", OperationsScreen)}
                {createStackScreen("Home", "", HomeScreen)}
